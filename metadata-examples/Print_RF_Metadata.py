...
def getTypesFromList(types_list):
    types = []
...
def print_set_type(attrs):
    ...
    if attrs['type']['kind'] == "set": 
        ...
        if 'name' in attrs['type']['type']:
            ...
            print "  "+attrs['name']+": set(union("+getTypesFromList(attrs['type']['type']['types'])+"))"
        else:
...
def print_union_types(attrs):
    ...
    if attrs['type']['kind'] == "union": 
        ...
        union_string = "  "+attrs['name']+" union("+getTypesFromList(attrs['type']['types'])+")"
        print union_string
